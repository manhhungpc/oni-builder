...
function compressBuildingConnectionData(connections: Map<string, NodeData>): Compressed {
...
function decompressBuildingConnectionData(compressedData: Compressed): Map<string, NodeData> {
    try {
        const decompressedData = decompress(compressedData) as { [key: string]: SimplifiedNode };
        const connections = new Map<string, NodeData>();
