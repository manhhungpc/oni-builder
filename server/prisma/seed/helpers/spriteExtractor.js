...
async function sharpImage(textureFilePath, sprite, matchModifier) {
    ...
    // Special handling for _None modifiers
    if (matchModifier.name.endsWith('_None')) {
        const trimmedBuffer = await itemImage.toBuffer({ resolveWithObject: true });
        const extendWidth = Math.max(trimmedBuffer.info.height, trimmedBuffer.info.width);

        // Extend the image with padding on all sides to make it 3x its current size
        itemImage = sharp(trimmedBuffer.data).extend({
            top: extendWidth,
            bottom: extendWidth,
            left: extendWidth,
            right: extendWidth,
